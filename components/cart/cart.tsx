...
// Mức ngưỡng vượt để có free shipping
...
const Cart = () => {
  ...
  useEffect(() => {
    ...
    // Bước này là sync với user sau khi render ui xong nè
    initCart();
  }, []);
  ...
  return (
    <>
      ...
      {isOpen && (
        <div
          className="fixed inset-0 bg-black/60 bg-opacity-50 z-50 transition-opacity backdrop-blur-sm"
          onClick={close}
        />
      )}
      ...
      <div
        className={`
                    fixed right-0 top-0 h-full w-full sm:w-100 bg-white shadow-2xl
